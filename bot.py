...
async def download_video(url: str, uid: int):
    def run():
        for attempt in range(3):
            try:
                opts = get_ydl_opts({
                    'outtmpl': str(Config.DOWNLOADS_PATH / f"v_{uid}_{int(time.time())}_{attempt}_%(title)s.%(ext)s"),
...
async def download_mp3(url: str, uid: int):
    def run():
        for attempt in range(3):
            try:
                opts = get_ydl_opts({
                    'outtmpl': str(Config.DOWNLOADS_PATH / f"a_{uid}_{int(time.time())}_{attempt}_%(title)s.%(ext)s"),
